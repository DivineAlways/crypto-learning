...
import { fetchHistoricalPrices } from "@/app/actions"
...
export function StockChart({ symbol, name = "Loading...", currentPrice, percentChange }: StockChartProps) {
  ...
  useEffect(() => {
    async function loadData() {
      setLoading(true)
      setError(null)
      const { data, error } = await fetchHistoricalPrices(symbol)
      if (error) {
        ...
        setData(
          data.map((item: any) => ({
            ...
            price: item.price || item.close, // Handle both CoinGecko and Tiingo data formats
          })),
        )
      } else {
